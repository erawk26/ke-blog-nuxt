      ...
      v-form(ref='availability' v-on:submit.prevent)
        v-stepper(v-model='stepper', vertical='')
          ...
          v-stepper-content(step='1')
            ...
            v-btn.ml-2(outlined color='success', @click='stepper += 1' :disabled="firstName.length<=2") Next
          v-stepper-step(:complete='stepper > 2', step='2' @click="stepper=Math.min(stepper,2)") Select Availability
          v-stepper-content(step='2')
            scheduler#scheduler(ref="scheduler" v-on:change="selectionChanged" :cal-start="calendar.start" :cal-end="calendar.end" :day-start="workDay.start" :day-end="workDay.end" :days="numOfDays" :iso-start="startDay")
            ...
            .eo-flex.j-start.a-center.mt-5
              ...
              v-btn.mr-2(outlined color='success', @click='stepper += 1' :disabled="datesArr.length===0") Next
          v-stepper-step(:complete='stepper > 3', step='3') Confirm Info
          v-stepper-content(step='3' @click="stepper=Math.min(stepper,3)")
            v-card.unstyle.pa-2.mb-5.output.eo-flex.a-stretch.wrap(elevation="4" tag="ul" key="schedule" v-if="schedule.length")
            ...
            .eo-flex.pt-2.two-cols.a-start
              v-btn.mr-2(outlined color='warning', @click='stepper -= 1') Prev
              v-btn.mr-2.bold(outlined color="error" @click='reset') Clear
              v-btn.mr-2.bold(:disabled="!canSubmit" outlined color='success' @click='submitForm') Send
